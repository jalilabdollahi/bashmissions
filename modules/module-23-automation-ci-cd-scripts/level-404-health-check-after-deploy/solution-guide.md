# Guide for Health Check After Deploy

Try building the script in this order:

1. Read the input file path from `$1`.
2. Exit with status `1` and print nothing if the file does not exist.
3. Print `health-check-after-deploy:404:processed:3` when the file exists.
...
output='health-check-after-deploy:404:processed:3'
